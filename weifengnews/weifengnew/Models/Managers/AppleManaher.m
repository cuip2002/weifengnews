...
-(void)loadApple:(int)pageIndex
{
    ...
    NSString * urlString = [NSString stringWithFormat:@"http://www.weiphone.com/apple/news/index_%d.shtml",pageIndex];
...
- (void)getResolvingData:(NSMutableData *)webData
{
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(getAppleMessageArray:)])
    {
        [self.delegate getAppleMessageArray:entityArr];
    }
    [entityArr release];
}
